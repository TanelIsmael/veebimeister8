...
class Support {
	...
	public function __construct() {
		$this->page_id     = 'wp_quiz_support';
		$this->parent_slug = 'edit.php?post_type=' . PostTypeQuiz::get_name();
		$this->page_title  = esc_html__( 'Get Support for WP Quiz', 'wp-quiz' );
		$this->menu_title  = esc_html__( 'Support', 'wp-quiz' );
	...
	public function load() {
		...
		add_meta_box(
			'support-content',
			esc_html__( 'Technical Support', 'wp-quiz' ),
			array( $this, 'support_content' ),
			$screen->id,
			'normal',
			'core'
		);

		// Needed javascript to allow drag/drop, expand/collapse and hide/show boxes.
		wp_enqueue_script( 'common' );
		wp_enqueue_script( 'wp-lists' );
		wp_enqueue_script( 'postbox' );

		wp_enqueue_script( 'clipboard', wp_quiz()->admin_assets() . 'js/clipboard.min.js', array( 'jquery' ), '1.7.1', true );
		wp_enqueue_script( 'wp-quiz-support', wp_quiz()->admin_assets() . 'js/support.js', array( 'clipboard', 'wp-quiz-admin' ), '2.0.0', true );
	}
	...
	public function render() {
		...
		<div class="wrap" id="config-page">
			...
			<div id="poststuff">
				<div id="post-body" class="metabox-holder <?php echo esc_attr( $columns_css ); ?>">
	...
	public function support_content() {
		...
		<div>
			<p>
				...
				printf( __( 'We offer technical support through our %s. Please <strong>copy and paste the following information in your ticket</strong> when contacting support:', 'wp-quiz' ), '<a href="https://community.mythemeshop.com/forum/12-plugin-support/" target="_blank">' . __( 'Support Ticket System', 'wp-quiz' ) . '</a>' ); // WPCS: xss ok.
				?>
			</p>

			<textarea style="font-family: monospace;" class="large-text" id="mts-debug-data-field" rows="16" readonly="readonly" disabled><?php esc_html_e( 'Generating environment information. Please wait. . .', 'wp-quiz' ); ?></textarea>
	...
	/**
	 * Gets debug log content.
	 */
	public function get_debug_log() {
		echo $this->debug_data_output(); // WPCS: xss ok.
		die();
	}
	 ...
	 *
	 * @param bool $copy_button Copy button or not.
	 */
	public function support_buttons( $copy_button = true ) {
		?>
		<div class="mts-help-buttons">
			<?php if ( $copy_button ) { ?>
				<button type="button" class="button mts-support-copy" disabled>
					<!-- <span class="dashicons dashicons-clipboard"></span> -->
					<?php esc_html_e( 'Copy Data for Support Request', 'wp-quiz' ); ?>
				</button>
			<?php } ?>
			<a href="https://community.mythemeshop.com/forum/12-plugin-support/" target="_blank" class="button button-primary mts-support-link">
				<!-- <span class="dashicons dashicons-external"></span> -->
				<?php esc_html_e( 'Open Support Forum', 'wp-quiz' ); ?>
			</a>
		</div>
		<?php
	}
	...
	/**
	 * Gets debug section output.
	 *
	 * @param array $debug_section Debug section data.
	 * @return string
	 */
	...
	/**
	 * Gets debug data in prettify format.
	 *
	...
	public function get_debug_data() {
		...
		/**
		 * Allows adding support log data.
		 *
		 * @param array $data Support log data.
		 */
	...
	public function get_environment_info() {
		...
		if ( function_exists( 'curl_version' ) ) {
			$curl_version = curl_version(); // phpcs:ignore.
			$curl_version = $curl_version['version'] . ', ' . $curl_version['ssl_version'];
		}
		return array(
			'server_info'               => $_SERVER['SERVER_SOFTWARE'], // phpcs:ignore
	...
	public function get_wp_info() {
		...
		if ( function_exists( 'memory_get_usage' ) ) {
			$wp_memory_limit = max( $wp_memory_limit, $this->let_to_num( @ini_get( 'memory_limit' ) ) ); // phpcs:ignore
		}
		return array(
